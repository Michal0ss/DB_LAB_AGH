...
// UPDATE
db.accounts.updateOne( {_id: 0}, {"$set": {balance: NumberDecimal(24)} } )
db.customers.replaceOne( {_id: 1},
	{
		_id: 1,
		firstname: "Marek",
		lastname: "Nowak",
...
// Pracownicy, którzy mają dwa numery telefonu
db.employees.find({ Phone: { $size: 2 } });

// Pracownicy o imieniu Steven
db.employees.find({FirstName: "Steven"})

// Pracownicy na 'A
db.employees.find({FirstName: /^A/})

// Pracownicy z Wielkiej Brytanii, zarabiający więcej niż 1200
db.employees.find({"Address.Country": "UK", Salary: {$gt: 1200}})
select * from employees where Address.Country = "UK" and Salary > 1200

// Pracownicy zatrudnieni po 1992, z UK, albo mający w nazwie stanowiska "Representative" albo zarabiający więcej niż 1500
db.employees.find({
  "Address.Country": "UK",
  HireDate: { $gt: ISODate("1992-01-01T00:00:00.000Z") }
  $or: [
...
```js
// Średnie zarabki pracowników w poszczególnych krajach
db.employees.aggregate([
  {
    $group: {
      _id: "$Address.Country",
  ...
  },
  {
    $project: {
      _id: 0,
      Country: "$_id",
      "Average salary": { $round: ["$Average salary", 2] },
    },
  },
  {
    $sort: { "Average salary": -1 },
  },
]);

// Największa pensja z podziałem na stanowisko
db.employees.aggregate([
  {
    $group: {
      _id: "$Title",
  ...
  },
  {
    $project: {
      _id: 0,
      Title: "$_id",
      "Highest salary": { $round: ["$Highest salary", 2] },
    },
  },
]);

// Liczba pracowników urodzona w danym roku
db.employees.aggregate([
  {
    $group: {
      _id: { $year: "$BirthDate" },
      "Employee counter": { $sum: 1 },
    },
  },
  {
    $project: {
